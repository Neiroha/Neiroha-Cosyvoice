...
TEXT = {
    "zh": {
        ...
        "download_assets": "下载资产",
        "download_asset": "资产",
        "download_target": "目标路径",
        "download_expected_size": "预计大小",
        "download_overwrite": "覆盖行为",
        "stop_download": "停止下载",
        "log_source": "日志源",
        "auto_refresh": "自动刷新",
        "runtime_status": "运行状态",
    ...
    "en": {
        ...
        "download_expected_size": "Expected Size",
        "download_overwrite": "Overwrite Behavior",
...
DOWNLOAD_ASSETS = {
    "cosyvoice3": {
        "name": "CosyVoice3 base model",
        "target": "models/Fun-CosyVoice3-0.5B",
        "expected_size": "multi-GB; see source metadata",
        "sources": {
            "modelscope": "FunAudioLLM/Fun-CosyVoice3-0.5B-2512",
            "hf": "FunAudioLLM/Fun-CosyVoice3-0.5B-2512",
            "hf-mirror": "hf-mirror: FunAudioLLM/Fun-CosyVoice3-0.5B-2512",
        },
    },
    "wetext": {
        "name": "wetext frontend",
        "target": "models/_cache/modelscope/models/pengzhendong/wetext",
        "expected_size": "small frontend resource; see source metadata",
        "sources": {
            "modelscope": "pengzhendong/wetext",
            "hf": "modelscope: pengzhendong/wetext (forced by downloader)",
            "hf-mirror": "modelscope: pengzhendong/wetext (forced by downloader)",
        },
    },
    "ttsfrd": {
        "name": "CosyVoice ttsfrd frontend",
        "target": "models/CosyVoice-ttsfrd",
        "expected_size": "large frontend resource; see source metadata",
        "sources": {
            "modelscope": "iic/CosyVoice-ttsfrd",
            "hf": "FunAudioLLM/CosyVoice-ttsfrd",
            "hf-mirror": "hf-mirror: FunAudioLLM/CosyVoice-ttsfrd",
        },
    },
}
...
def _format_settings(api_base: str, admin_url: str, registry: VoiceRegistry) -> str:
    config = registry.server_config()
    api_config = config.get("api", {}) if isinstance(config.get("api"), dict) else {}
    admin_config = config.get("admin", {}) if isinstance(config.get("admin"), dict) else {}
    startup_config = config.get("startup", {}) if isinstance(config.get("startup"), dict) else {}
    security_config = config.get("security", {}) if isinstance(config.get("security"), dict) else {}
    api_key_state = "configured" if first_non_empty(security_config.get("api_key")) else "not configured"
    rows = [
        ("API URL", api_base),
        ("Admin URL", admin_url),
        ...
        ("Config file", profile_path_text(SERVER_CONFIG_PATH)),
        ("Restart", "Stop the launcher and run pixi run serve after config edits."),
    ]
    table = ["| Setting | Value |", "| --- | --- |"]
    table.extend(f"| {key} | {value} |" for key, value in rows)
    return "\n".join(table)
...
def _format_download_assets(source: str, force: bool, text: dict[str, str]) -> str:
    is_zh = text.get("download_assets") == "下载资产"
    overwrite = (
        "请求强制刷新；下载器会刷新元数据并复用有效本地文件"
        if force and is_zh
        else "Force requested; downloader refreshes metadata and reuses valid local files"
        if force
        else "复用已有有效文件"
        if is_zh
        else "Reuse existing valid files"
    )
    rows = [
        (
            asset["name"],
            asset["sources"].get(source, asset["sources"]["modelscope"]),
            asset["target"],
            asset["expected_size"],
            overwrite,
        )
        for asset in DOWNLOAD_ASSETS.values()
    ]
    table = [
        (
            f"| {text['download_asset']} | {text['download_source']} | "
            f"{text['download_target']} | {text['download_expected_size']} | "
            f"{text['download_overwrite']} |"
        ),
        "| --- | --- | --- | --- | --- |",
    ]
    table.extend(f"| {name} | {source_id} | `{target}` | {size} | {behavior} |" for name, source_id, target, size, behavior in rows)
    return "\n".join(table)
    ...
    *,
    ...
    def save_voice(
        ...
        try:
            ...
            return str(exc), _format_voice_sets(registry)
    ...
    with gr.Blocks(
        ...
        with gr.Tab(t("voice_sets")):
            ...
            gr.Button(t("refresh"), variant="secondary").click(lambda: _format_voice_sets(registry), outputs=voice_sets_box)
        ...
        with gr.Tab(t("download")):
            ...
            download_assets = gr.Markdown(
                value=_format_download_assets("modelscope", False, text),
                label=t("download_assets"),
                elem_classes=["neiroha-table"],
            )
            with gr.Row():
            ...
            download_source.change(lambda source, force: _format_download_assets(source, force, text), inputs=[download_source, download_force], outputs=download_assets)
            download_force.change(lambda force, source: _format_download_assets(source, force, text), inputs=[download_force, download_source], outputs=download_assets)
        ...
        with gr.Tab(t("logs")):
            ...
            with gr.Row():
                auto_refresh = gr.Checkbox(value=False, label=t("auto_refresh"))
            ...
            log_timer = gr.Timer(value=2.0, active=False)
            auto_refresh.change(lambda enabled: gr.update(active=enabled), inputs=auto_refresh, outputs=log_timer)
            log_timer.tick(read_selected_log, inputs=log_source, outputs=log_box)

        with gr.Tab(t("settings")):
            settings_box = gr.Markdown(
                value=_format_settings(api_base, admin_url, registry),
                elem_classes=["neiroha-table"],
            )
            gr.Button(t("refresh"), variant="secondary").click(
                lambda: _format_settings(api_base, admin_url, registry),
                outputs=settings_box,
            )
